...
def _make_divisible(v, divisor, min_value=None):
    ...
    if min_value is None:
        min_value = divisor
    # int(v + divisor / 2) // divisor * divisor (half adjust)
...
class MobileNetSeg(nn.Module):
    def __init__(self, base_name, heads,head_conv=24, pretrained = True):
        # mobilenetv2_5, {'hm':1, 'reg':2, 'wh':2} 24 
...
def mobilenetv2_5(pretrained=False, **kwargs):
    ...
    if pretrained:
        pretrained_model_path = os.path.join(config.ROOT, 
            "models/pretrained/mobilenetv2_backbone.pth")
        assert os.path.exists(pretrained_model_path), "not exists pretrain model: `{}`".format(pretrained_model_path)
        model.load_state_dict(torch.load(pretrained_model_path))
        #print('This version does not have pretrain weights.')
    return model

# num_layers  : [10 , 5]
def get_mobile_net(num_layers, heads, head_conv=24, pretrained=True):
...
def MobileNet(num_layers = 5, 
              num_classes=1,
              head_conv = 24, pretrained=True):

    heads = {'heatmap':num_classes, 'offset':2, 'wh':2}
    model = get_mobile_net(num_layers,
                   heads,
                   head_conv,
                   pretrained=pretrained)
    return model
...
if __name__ == '__main__':
    ...
    model = get_mobile_net(5,{'hm':2, 'ang':1, 'offset':2,'wh':2},head_conv=24)
