...
if __name__ == '__main__':
    ...
    infer_images_dir = 'data/ZJB_dataset/light_data/images'
    ...
    image_list = os.listdir(infer_images_dir)
    for name in image_list:
        if not name.endswith('jpg'):
            continue

        if name.split('.')[-1] == 'jpg':
            image_path = os.path.join(infer_images_dir, name)
            ...
            cv2.imwrite(os.path.join(output_dir, name), draw_image)
